...
Login ──(register link)──> Register ──(success, auto-login)──┐
  ...
  └───────────────────────────────────────────────────────────┴──> Home
                              ...
                              │ tap "Flash Card"   │ tap "Ngữ Pháp"   │ tap "Luyện Tập"                │ tap "Kiểm tra"
                              ▼                    ▼                  ▼                                 ▼
                         FlashCardScreen      GrammarScreen      PracticeScreen (4 exercise cards)   QuizScreen
                 (tap "Nhóm từ" → CreateGroupModal popup)                │ tap an exercise card
          (tap "Dò bài ngẫu nhiên" → RandomReviewModal popup)            ▼
                              │ tap an existing group card      PracticeExerciseScreen
                              ▼
                       FlashCardStudyScreen
```

- Login and Register both land on **Home** directly on success (register auto-logs-in; it no longer bounces back to Login).
- All 4 `TopicDetailScreen` menu items are built out — there is no placeholder screen left in the app. If a future menu item is added before it's built, give it a real stub screen (tap feedback + a labeled back button) rather than a dead click; the app no longer ships a generic `SuccessScreen` for this since nothing uses it.
- `src/screens/GrammarScreen.tsx` (opened by tapping "Ngữ Pháp" on `TopicDetailScreen`) renders the one grammar lesson mapped 1:1 to the current topic — cách dùng, cấu trúc table, dấu hiệu nhận biết, and 5 example sentences — looked up via `getGrammarLesson(topicName)` from `src/data/grammar.ts`.
- `src/screens/PracticeScreen.tsx` (opened by tapping "Luyện Tập") lists that topic's 4 exercises ("Bài 1"–"Bài 4"); tapping one opens `src/screens/PracticeExerciseScreen.tsx`, a 10-sentence Vietnamese→English translation quiz. "Nộp bài" first checks every field is filled (inline error, not `Alert`), then grades via case-insensitive, trailing-punctuation-insensitive exact-string match against the canonical `english` answer (`normalize()` in that file) — **there is no fuzzy/semantic grading**, since the app has no backend/LLM to call at runtime; a correct translation phrased differently from the stored answer will be marked wrong. Wrong answers reveal the correct sentence; "Làm lại" resets the same 10 sentences (not a re-roll, unlike `RandomReviewModal`).
...
- `src/screens/FlashCardStudyScreen.tsx` is the actual flip-card study view for one word group (opened by tapping a group card in `FlashCardScreen`): a two-sided card (front = word/phonetic/example in the active direction, back = the translation, flipped via `Animated` rotateY), a single `PanResponder` handling both tap-to-flip and horizontal swipe-to-navigate (swipe right = next word, swipe left = previous — matches the user's stated direction, not the more common inverse), an EN-VN/VN-EN direction toggle, a position counter, and a shuffle toggle that reshuffles `order` (an index array separate from the group's stored `wordIndices`, so shuffling never mutates storage). Word audio uses `expo-speech`'s `Speech.speak(text, { language: 'en-US' })` — the only "audio" dependency in the app, still fully on-device (TTS, no network/API call). The `PanResponder` is recreated on every render (not frozen behind `useRef`) so its closures always see current `position`/`flipped` state — freezing it caused a real bug once (swipes only advanced one step, tap-to-flip never fired since `onStartShouldSetPanResponder` returned `false`), don't reintroduce that pattern.
- `src/components/RandomReviewModal.tsx` (opened from `FlashCardScreen`'s "Dò bài ngẫu nhiên" button) quizzes on 10 random words from the **whole topic** (not just one word group): each word is randomly presented either as its Vietnamese meaning or as a speaker button (`expo-speech`), and the user must type the English word either way. "Hoàn Thành" validates every field is filled (inline error, not `Alert`, per the web no-op gotcha below) before case-insensitive exact-match grading; wrong answers reveal the correct spelling after grading, and "Làm lại" re-rolls a fresh 10.
- `src/screens/QuizScreen.tsx` (opened by tapping "Kiểm tra") is the pass/fail test for one topic: Phần 1 picks 5 random words from that topic's vocab (same meaning/audio-prompt style as `RandomReviewModal`, sharing its shuffle logic via `src/utils/shuffle.ts`'s `shuffledSlice()`); Phần 2 picks 5 random sentences from that topic's 10-sentence bank in `data/Quiz.json` (same VN→EN translation UI, `normalize()` grading, and `extraVocabulary` hint rendering as `PracticeExerciseScreen`). "Nộp bài kiểm tra" first checks every field is filled, then grades both parts — **passing requires all 5 vocab AND all 5 sentences correct**, not a partial score; a pass calls `markTopicPassed(topicName)` from `src/data/progress.ts` (idempotent — retaking an already-passed topic's quiz doesn't double-count) and shows a green banner, a fail shows a red banner with wrong answers revealed; "Làm lại" re-rolls both parts fresh. `TopicDetailScreen` shows a green checkmark badge on the "Kiểm tra" menu item once that topic is passed.
- `src/data/progress.ts` is the real (not hardcoded) implementation of the header progress bar: `loadProgress()` reads the passed-topics list from AsyncStorage once at app mount (`App.tsx`), `markTopicPassed()` persists a new pass, and progress % = `passedTopics.length * 4` (25 topics × 4% = 100%). Screens must use the `useProgressPercent()` hook (not the plain `getProgressPercent()` getter) so the header re-renders live when a quiz is passed — it subscribes to an in-module pub-sub of listeners notified on every `loadProgress()`/`markTopicPassed()` call. `isTopicPassed(topicName)` is a synchronous read of the same in-memory cache, used for the checkmark badge.
...
- `data/Grammar.json` — one grammar lesson per topic (25 total), transcribed from `data/Giao_an_ngu_phap_tieng_Anh.pdf` and mapped to topics **by list order** (lesson N ↔ topic N in `Topic_vocabulary.json`; the two are not otherwise thematically related, since the source PDF's 25 grammar points are generic English tenses/structures, not topic-specific). Shape: `[{ topic, title, titleEnglish, usage: string[], structure: [{ label, formula }], signalWords, examples: [{ sentenceEng, sentenceMeaning }] }]`. Loaded via `src/data/grammar.ts`'s `getGrammarLesson(topicName)`.
- `data/Practice.json` — 4 exercises × 10 Vietnamese→English sentences per topic (1000 sentences total), generated (not hand-written) to use only that topic's vocabulary from `Topic_vocabulary.json` and that topic's one grammar structure from `Grammar.json`. Shape: `[{ topic, exercises: [{ title, sentences: [{ vietnamese, english, extraVocabulary: [{ word, meaning }] }] }] }]`. `extraVocabulary` (~43% of sentences have a non-empty one) lists real content words the sentence needed beyond the topic's own vocab list, each paired with a Vietnamese `meaning` accurate to the sense used in that specific sentence (e.g. "shift" → "ca làm việc" in a work context, not "sự thay đổi") — grammar scaffolding (pronouns/articles/be-forms/modals/prepositions) is never listed here. `PracticeExerciseScreen` renders each as "word (meaning)" under the Vietnamese prompt. Loaded via `src/data/practice.ts`'s `getTopicPractice(topicName)`. If ever regenerated, keep the same per-topic vocab/grammar constraint and re-derive `extraVocabulary` with context-accurate meanings — that's the entire point of gating practice content per topic instead of pulling generic sentences.
- `data/Quiz.json` — the "Kiểm tra" question bank: 10 Vietnamese→English sentences per topic (250 total, separate content from `Practice.json`, not reused from it), same per-topic vocab/grammar constraint and same `extraVocabulary: [{ word, meaning }]` shape as `Practice.json`. Shape: `[{ topic, sentences: [{ vietnamese, english, extraVocabulary }] }]` (flat per-topic list, no exercise grouping — `QuizScreen` randomly samples 5 of the 10 at test time). Loaded via `src/data/quiz.ts`'s `getTopicQuiz(topicName)`.
