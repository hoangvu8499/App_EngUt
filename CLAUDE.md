...
Login ──(register link)──> Register ──(success, auto-login)──┐
  ...
  └───────────────────────────────────────────────────────────┴──> Home
                                              ...
                                              │ tap "Flash Card"   │ tap "Ngữ Pháp"   │ tap "Luyện Tập"
                                              ▼                    ▼                  ▼
                                         FlashCardScreen      GrammarScreen      PracticeScreen (4 exercise cards)
...
- Any not-yet-built destination (Kiểm tra menu item on TopicDetailScreen) routes to the generic `SuccessScreen` placeholder (shows "SUCCESS" + subtitle + a labeled back button). This is the standing pattern for stubbing out future features — always give tap feedback and a way back, never a dead click.
